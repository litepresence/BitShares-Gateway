...
def initialize_addresses(network):
    """
...
def unlock_address_process(network, idx, delay):
    """
    check the binary state of the gateway addresses
...
def unlock_address(network, idx, delay):
    """
    a process wrapper to delay unlocking an address
    """
    unlock = Process(target=unlock_address_process, args=(network, idx, delay,))
    unlock.start()
